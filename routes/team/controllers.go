...
func UpImage(w http.ResponseWriter, req *http.Request) (ImgResp,Shared.Error) {
	// req.MultipartReader()
	resp := ImgResp{}
	fmt.Println(10)
...
// }
	...
	if err != nil{
		panic(err)
		return resp,Shared.Error{Message: "Internal Server Error."}
	}
	defer file.Close()
	...
	if(handler.Header.Get("Content-Type") != "image/jpeg"){
		...
		return resp,Shared.Error{Message: "Upload jpg image"}
	}
	...
	w.WriteHeader(http.StatusOK)
	return resp, Shared.Error{Message: ""}
